...
// AddGroupMembersScreen: UI to allow group admins to add friends to an existing group
...
class _AddGroupMembersScreenState extends State<AddGroupMembersScreen> {
  // Services used to fetch friends and update group
  final ChatService _chatService = ChatService();
  final GroupService _groupService = GroupService();
  final FirebaseAuth _auth = FirebaseAuth.instance;

  // Local state
  List<UserModel> _friends = []; // friends that can be added (excluding existing members)
  final Set<String> _selectedUserIds = {}; // selected friend UIDs to add
  bool _loading = true; // true while loading friend candidates
  bool _submitting = false; // true while adding members
  ...
  // Load friends and filter out current group members
  ...
  // Add selected members to the group (only callable by admins)
  Future<void> _addSelectedMembers() async {
    ...
    // Only admins allowed to add members
    ...
    setState(() {
      _submitting = true; // disable UI while submitting
    });
    try {
      ...
      if (mounted) {
        // Return to previous screen signaling success
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // ADD button in app bar (also disabled while submitting)
          TextButton(
      ...
      body: _loading
      // Show loader while fetching friends
          ? const Center(child: CircularProgressIndicator())
      // If no friends available to add
          : _friends.isEmpty
          ? const Center(child: Text('No friends available to add'))
      // Otherwise show selectable list of friends
          : ListView.separated(
      ...
      // Bottom action bar showing Add button with selected count
